...
2. [ ] **Arquitectura de equipos/workspace** — el cambio más grande: todo hoy cuelga de `owner_id` (single-owner). Migrar a un modelo de `organization`/`workspace` con membresía (owner/admin/member) y RLS por equipo, no por dueño único. Billing por workspace (estilo Slack), no por usuario individual. **No iniciado.**
3. [ ] **Freemium + Onboarding** — gating de funciones de IA (y otros topes: empresas/proyectos/asientos, por decidir con datos reales de costo) para el plan gratuito; flujo de onboarding que levante rol/retos/objetivos del usuario nuevo. **No iniciado**, depende de (2).
